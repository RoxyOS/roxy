...
1. Edit the package's working copy at `distro/sources/<name>-workdir` **in place**. This is the
   ...
   which is then applied at build time. Never hand-edit `distro/sources/<name>` or `-clean`:
   `<name>` is the build tree, overwritten by regen; `-clean` is upstream plus the official
   patches (regenerated by prepare). Edits there are discarded or poison the generated patch.
   Never hand-write or directly edit a package patch — always generate it from the `-workdir`
   edits via `jinx regen`.
