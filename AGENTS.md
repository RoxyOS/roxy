...
- Read `agent-instructions/STYLE.md` before changing source code. It defines repository-wide coding
  conventions.
- Read `agent-instructions/WORKFLOW.md` for task-specific procedures, including mlibc development,
  distro package patching, source/recipe updates, and rootfs validation.
- Read `agent-instructions/REVIEW.md` before reviewing changes. It defines the required review
  scope, checks, and finding format.
- Read `agent-instructions/PLAN.md` before writing an implementation plan. It defines the required
  investigation, content, and validation detail.
...
- Treat `agent-instructions/STYLE.md` as the sole source of truth for coding style; consult it
  instead of duplicating style rules in this file.
