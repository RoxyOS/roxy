//! Linux evdev ABI constants that appear in event payloads and capability bitmaps.
//!
//! Sourced from Linux `include/uapi/linux/input-event-codes.h`. These are *protocol data
//! values* (event type, key code, LED identifier, synchronisation code), not ioctl request
//! numbers. The request numbers (`EVIOCGVERSION`, `EVIOCGBIT`, …) belong in
//! `kernel/syscall/src/syscalls/ioctl/evdev_abi.rs`.
