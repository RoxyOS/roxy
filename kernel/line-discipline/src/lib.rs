...
impl LineDiscipline {
    ...
    pub fn process(&mut self, input: &[u8]) -> ProcessResult {
        ...
        // Interrupt character (Ctrl+C) generates SIGINT independently of canonical mode.
        if self.settings.isig && input == [self.settings.intr_character] {
            return ProcessResult {
                echo: false,
                buffer: None,
                signal: Some(Signal::Interrupt),
            };
        }
...
pub struct ProcessResult {
    ...
    /// A signal to deliver to the foreground process, generated by a control character.
...
mod tests {
    ...
    kernel_test!(
        "roxy-line-discipline::interrupt",
        generates_interrupt_signal,
        {
            let mut discipline = LineDiscipline::new();

            assert_eq!(
                discipline.process(b"\x03"),
                ProcessResult {
                    echo: false,
                    buffer: None,
                    signal: Some(Signal::Interrupt),
    ...
    kernel_test!(
        "roxy-line-discipline::interrupt-disabled",
        treats_interrupt_as_ordinary_input,
        {
