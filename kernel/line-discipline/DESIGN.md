...
newline event commits the line.
Backspace byte `0x08` removes the final complete UTF-8 scalar and is never committed; an empty
buffer ignores it without echo. A committed line includes its newline and is moved into the
result's buffer, leaving the editing buffer empty for the next line. Escape sequences otherwise
remain ordinary buffered bytes.
...
not echoed, while backspace and newline remain available. Noncanonical mode returns each accepted
event immediately in the result buffer and does not interpret backspace or newline. The owning TTY
shares one discipline instance and its settings across all open files.

## Limits

Userspace termios configuration, configurable control characters, EOF, line kill, signal
